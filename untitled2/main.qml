...
Window {
    ...
    Image {
        ...
        Register{
			...
			visible: ui.state == "recognize" || ui.state == "autoRecognize"
            focus: ui.state == "recognize" || ui.state == "autoRecognize"
			onVisibleChanged: {
                if (visible) {
                    idNumber.text = ""
                    name.text = ""
        ...
        Column {
            ...
            spacing: 6
            visible: ui.state != "start" && ui.state != "calibration"
        ...
        Button {
            ...
            height: 73
            opacity: 0.9
            visible: ui.state != "start" && ui.state != "autoRecognize"      
        }
		...
		Keys.onPressed: {
            ...
            switch(event.key){
            case Qt.Key_Return:
                if (ui.state == "register" ){
                    interFace.SetPerson(pwString)
                    interFace.TriggerRegister()
                }else if(ui.state == "recognize"){
                    interFace.TriggerRecognize()
                }else if(ui.state == "pointCloud"){
					ui.state = "register"
				}
	...
	Connections{
				target: interFace
				onRecognizeSuccess:{
					ui.state="recognizeSucceed"
				}

				onRecognizeFailed:{
					ui.state="recognizeFaild"
				}
